...
#if defined(NDEBUG)
# error "Folkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc78bf4c9ae0138e7339b7b765ba59cd25a77d59b42a26652274f0eeae39a3275");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Folkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Folkcoin Signed Message:\n";
...
{
    // Folkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 350000); // Folkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 12 * 24 * 60 * 60; // Folkcoin: 10 days
static const int64 nTargetSpacing = 8 * 60; // Folkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Folkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Folkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe81dd7f699340fe943971ec42f7997b6033110c90ce22587d75e7dbe37452048");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Get out of the way or be part of the journey.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0407dd2d62e6055df4935b0fa19356a1a6402c1c91cea03a90bb2dd1ad932df47883142492d68d37f15d796b0f85b44746a06099b32e0b3c129f1427c574ee2256") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1620792654;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084612863;

        if (fTestNet)
        {
            block.nTime    = 1620792555;
            block.nNonce   = 387157316;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd67f146592100bd8b6e2f7b248dfbbdaf070d0e25a85c302da098508c4519c26"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc4, 0xb8, 0xdc }; // Folkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FolkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FolkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FolkcoinMiner(CWallet *pwallet)
{
    printf("FolkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("folkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FolkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FolkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FolkcoinMiner, pwallet));
}
